...
CREATE TABLE public.teacher (
	teacher_id serial4 NOT NULL,
     public_key public.xid NOT NULL DEFAULT xid(),
	"name" text NOT NULL,
	CONSTRAINT teacher_pk PRIMARY KEY (teacher_id),
	CONSTRAINT teacher_un UNIQUE (public_key)
);
CREATE INDEX teacher_public_key_idx ON public.teacher USING btree (public_key);
...
CREATE TABLE public.subject (
	...
	CONSTRAINT subject_un UNIQUE (public_key)
);
CREATE INDEX subject_public_key_idx ON public.subject USING btree (public_key);
...
CREATE TABLE public."group" (
	...
	CONSTRAINT group_un UNIQUE (public_key)
);
CREATE INDEX group_public_key_idx ON public.group USING btree (public_key);
...
CREATE TABLE public.student (
	...
	CONSTRAINT student_un UNIQUE (public_key)
);
CREATE INDEX student_public_key_idx ON public.student USING btree (public_key);
...
CREATE TABLE public.mark (
	...
	CONSTRAINT mark_un UNIQUE (public_key)
);
CREATE INDEX mark_public_key_idx ON public.mark USING btree (public_key);
